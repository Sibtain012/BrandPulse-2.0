...
const COLORS = {
    ...
    'Non-Complaint': '#10B981',  // Emerald Green
};
...
const ComplaintChart = ({ data, title, subtitle }) => {
    const total = data.reduce((sum, item) => sum + item.value, 0);

    if (!data || data.length === 0) {
        return (
            <div className="flex items-center justify-center h-80 bg-gray-50 rounded-xl">
                <p className="text-gray-400">No complaint data available</p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full">
            <h3 className="text-2xl font-bold mb-2 text-center text-gray-800">
                {title || 'Complaint Classification Analysis'}
            </h3>
