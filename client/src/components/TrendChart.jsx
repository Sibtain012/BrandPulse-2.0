...
const LINE_COLORS = { Positive: '#10B981', Neutral: '#6B7280', Negative: '#EF4444' };
...
const TrendChart = ({ requestId, platform }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError('');
        axios.get('/api/data/trend/' + requestId + '?granularity=' + granularity, {
        ...
        <div className="space-y-4">
                ...
                <div className="flex border border-light-200 rounded-lg overflow-hidden">
                    {GRANULARITIES.map(g => (
                        <button key={g.key} onClick={() => setGranularity(g.key)}
                            className={`px-4 py-1.5 text-sm transition-colors border-r border-light-200 last:border-r-0 ${granularity === g.key
            ...
            <div className="bg-white p-4 rounded-xl border shadow-sm">
                <p className="text-sm font-medium text-light-700 mb-4">
                    Sentiment over time — {platform === 'twitter' ? 'Twitter' : 'Reddit'}
                </p>
